...
import {
  creatorType,
  getAllProject,
} from "../../redux/reducer/projectManagementReducer";
...
export default function ProductManagement({}: Props) {
  ...
  const { allProject } = useSelector(
    (state: RootState) => state.projectManagementReducer
  );
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <>
          <NavLink to={`/dashboard/${record.id}`}>{text}</NavLink>
        </>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          ...
          >
            {" "}
            <EditOutlined style={{ color: "#0747a6" }} />
          </a>
          <a>
